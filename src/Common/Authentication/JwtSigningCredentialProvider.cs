...
internal sealed class JwtSigningCredentialProvider : IJwtSigningCredentialProvider {
        ...
        lock (_sync) {
            ...
            try {
                ...
                if (options.AllowDevSymmetricKey) {
                    ...
                    _cachedKey = CreateSymmetricKey(options.SecurityKey);
    ...
    private static SecurityKey CreateSymmetricKey(string? devKey) {
        ...
        if (devKey.Length < 32)
            throw new InvalidOperationException("Authentication:SecurityKey must be at least 32 characters for acceptable entropy.");
